...
适用于 ComfyUI 的词典式提示词自动生成节点，并内置用户词典生成工具链。

ComfyUI-PromptEngine 面向两类用户：

1. 希望自由组合提示词获得无限自动生成提示词的用户。21个维度的无限种组合，意味着无限种自由度，无限多的提示词，带来近乎无限的效果，再也不用苦心思考提示词。
2. 希望实现提示词结构化、可复用、可扩展的用户。本节点可帮助用户将手头积累的大量提示词按人物、外观、服装、姿态、构图、光线、风格等 21 个视觉维度拆分为可管理的词典条目，通过聚类整理成词典进行无限组合复用。

提示词自动生成节点提供两种主要使用方式：

1. 可串联的单维度节点，用户可自由选择需要的维度进行拼接，适合模块化搭建工作流；
2. 全维度一站式节点，适合快速完成整条 prompt。

对于需要持续扩充词典的用户，插件还内置 Step 1-3 工具链，可从原始 prompt 数据集中提取维度、聚类相近短语，并生成可在运行时与基础词典自动合并的用户增量词典。

## 提示词生成效果（模型使用z-image-turbo + LoRA）

![生成效果1](examples/4grid_001.png)
![生成效果2](examples/4grid_002.png)
![生成效果3](examples/4grid_003.png)
![生成效果4](examples/4grid_004.png)
